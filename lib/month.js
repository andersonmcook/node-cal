...
month.generateMonth = (year, month) => {
  ...
  const afterSpace = Math.ceil((22 - topLine.length) / 2 + 1);
  const space = " ";
  return `${space.repeat(spaceCount)}${topLine}${space.repeat(afterSpace)}`;
};
...
month.calendarBody = (year, month) => {
  ...
  days[today - 1] = `${today} _\b`;
  days[today] = `_\b${today + 1}`;
  days[0] = `${space.repeat(spaceCount)} 1`
  days = _.chunk(days, 7);
  console.log(days);
  // let body = `${space.repeat(spaceCount)}${days.join(" ")}`;
  // body = body.split("");
  // for (let i = 20; i < body.length; i += 21) {
  //       body[i] = "  \n";
  // };
